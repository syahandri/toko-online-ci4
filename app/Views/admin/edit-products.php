    ...
    <div class="row">
        <div class="col">
            <div class="card card-body shadow border-0">
                <form action="/web-admin/products/update" method="post" enctype="multipart/form-data" class="row" id="formAddProduct">
                    <?= csrf_field() ?>
                    <input type="hidden" name="id" value="<?= $products['product']['id'] ?>">
                    <input type="hidden" name="image_1Lama" value="<?= $products['product']['image_1'] ?>">
                    <input type="hidden" name="image_2Lama" value="<?= $products['product']['image_2'] ?>">
                    <input type="hidden" name="image_3Lama" value="<?= $products['product']['image_3'] ?>">
                    <div class="col-12 col-sm-6">
                        <div class="mb-2">
                            <label for="name" class="form-label">Nama Produk</label>
                            <input type="text" class="form-control text-capitalize <?= !$validation->hasError('name') ?: 'is-invalid' ?>" name="name" placeholder="Mis: Kaos" value="<?= old('name', $products['product']['name']) ?>" required>
                        ...
                        <div class="mb-2">
                            ...
                            <input type="text" class="form-control only-number <?= !$validation->hasError('price') ?: 'is-invalid' ?>" name="price" placeholder="Mis: 1000000" value="<?= old('price', $products['product']['price']) ?>" required>
                        ...
                        <div class="mb-2">
                            ...
                            <input type="text" class="form-control only-number <?= !$validation->hasError('discount') ?: 'is-invalid' ?>" name="discount" placeholder="Mis: 50" value="<?= old('discount', $products['product']['discount']) ?>">
                        ...
                        <div class="mb-2">
                            ...
                            <input type="text" class="form-control only-number <?= !$validation->hasError('weight') ?: 'is-invalid' ?>" name="weight" placeholder="Mis: 1000" value="<?= old('weight', $products['product']['weight']) ?>" required>
                    ...
                    <div class="col-12 col-sm-6">
                        <div class="mb-2">
                            ...
                            <input type="text" class="form-control only-number <?= !$validation->hasError('stock') ?: 'is-invalid' ?>" name="stock" placeholder="Mis: 100" value="<?= old('stock', $products['product']['stock']) ?>" required>
                        ...
                        <div class="mb-2">
                            ...
                            <select class="form-select <?= !$validation->hasError('category') ?: 'is-invalid' ?>" name="category" required>
                                ...
                                <?php foreach ($categories as $category) : ?>
                                    <option value="<?= $category['id'] ?>" <?= old('category', $products['product']['category_id']) == $category['id'] ? 'selected' : '' ?>><?= $category['name'] ?></option>
                                <?php endforeach ?>
                            </select>
                        ...
                        <div class="mb-0">
                            ...
                            <?php foreach ($sizes as $size) : ?>
                                <div class="form-check form-check-inline">
                                    <input class="form-check-input size <?= !$validation->hasError('size') ?: 'is-invalid' ?>" type="checkbox" name="size[][id]" value="<?= $size['id'] ?>" <?php foreach (old('size', $products['size']) as $old) : ?> <?= $old['id'] == $size['id'] ? 'checked' : '' ?> <?php endforeach ?>>
                        ...
                        <div class="mb-2">
                            ...
                            <?php foreach ($colors as $color) : ?>
                                <div class="form-check form-check-inline">
                                    <input data-style="<?= $color['name'] ?>" class="form-check-input color bg-<?= $color['name'] ?> <?= $color['name'] != 'white' ?: 'color-white' ?> <?= !$validation->hasError('color') ?: 'is-invalid' ?>" type="checkbox" name="color[][id]" value="<?= $color['id'] ?>" title="<?= $color['name'] ?>" <?php foreach (old('color', $products['color']) as $old) : ?> <?= $old['id'] == $color['id'] ? 'checked' : '' ?> <?php endforeach ?>>
                                </div>
                            <?php endforeach ?>
                    ...
                    <div class="col-12">
                        <div class="mb-2">
                            ...
                            <input type="hidden" id="detail" name="detail" value="<?= old('detail', $products['product']['detail']) ?>" required>
                    ...
                    <div class="col-12 col-md-8">
                        <div class="mb-2">
                            ...
                            <p class="my-0 small text-muted">*Kosongkan jika tidak ingin mengubah foto produk</p>
                        </div>
                    </div>
